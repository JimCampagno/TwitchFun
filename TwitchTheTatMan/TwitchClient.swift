...
extension Twitch {
    ...
    func generateJSON(withSession session: URLSession, request: URLRequest, handler: @escaping (JSONDictionary?) -> Void) {
        session.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                guard let data = data else { handler(nil); return }
                guard let json = try? JSONSerialization.jsonObject(with: data, options: .allowFragments) as! JSONDictionary else { handler(nil); return }
                handler(json)
            }
            }.resume()
    }
